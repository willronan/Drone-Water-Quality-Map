...
(function () {
  ...
  async function bootstrapData() {
    const cfg = window.APP_CONFIG || {};
    const dataApiUrl = cfg.DATA_API_URL || 'https://getwaterdata-c2cmb6g7cbcqhhch.canadacentral-01.azurewebsites.net/api/GetDroneData';
    ...
    try {
      ...
      if (filteredDates.length) {
        ...
        const selectedRows = state.dateIndex[state.selectedDate] || [];
        if (selectedRows.length > 0) {
          lastTimestamp = selectedRows[selectedRows.length - 1].timestamp;
        }   
      }
  ...
  function ingestNextPoint(row, paramName) {
    const rowKey = row.timestamp || `${row.latitude},${row.longitude},${row[paramName]}`;

    if (state.ingestedRowKeys.has(rowKey)) return;
    ...
    state.ingestedRowKeys.add(rowKey);

    if (state.livePoints.length < 3) return;
  }

    function rebuildMesh() {
      if (state.livePoints.length < 3) {
        window.WebGLLayerModule.clearMesh();
        return;
      }
  ...
  async function pollNewData() {
    ...
    try {
      ...
      const url = `${sinceUrl}?since=${encodeURIComponent(lastTimestamp)}`;
      ...
      for (const row of newRows) {
        ...
        if (!state.dateIndex[dateKey]) {
          state.dateIndex[dateKey] = [];
          state.dates.push(dateKey);

          state.dateMeta[dateKey] = {
            label: row.dateLabel || dateKey,
            year: dateKey.slice(0, 4)
          };

          if (!state.years.includes(dateKey.slice(0, 4))) {
            state.years.push(dateKey.slice(0, 4));
            state.years.sort();
            populateYearSelector(state.years);
            document.getElementById('yearSelect').value = state.selectedYear;
          }
        ...
        state.dateIndex[dateKey].push(row);
      }

      lastTimestamp = newRows[newRows.length - 1].timestamp;

      const selectedDate = state.selectedDate;
      const selectedParam = state.selectedParam;
      const currentRows = state.dateIndex[selectedDate] || [];

      let added = false;

      for (let i = 0; i < newRows.length; i++) {
        const row = newRows[i];
        if (row.dateKey !== selectedDate) continue;

        const beforeCount = state.livePoints.length;
        ingestNextPoint(row, selectedParam);
        const afterCount = state.livePoints.length;
